		...
		<Card dark {...rest}>
			<Tabs.Root defaultValue={snippets[0].filename}>
				<Tabs.List
					...
					className="flex flex-nowrap justify-between overflow-x-auto"
				>
					<div className="flex">
						<div className="flex h-10 items-center space-x-1.5 border-r border-b border-gray-900 px-4 dark:border-gray-900">
							<span className="h-3 w-3 rounded-full bg-slate-500 dark:bg-slate-600" />
							<span className="h-3 w-3 rounded-full bg-slate-500 dark:bg-slate-600" />
							<span className="h-3 w-3 rounded-full bg-slate-500 dark:bg-slate-600" />
						</div>
							...
							({ filename }, index) =>
								filename && (
									<Tabs.Trigger
										key={index}
										value={filename}
										className="relative flex h-10 shrink-0 items-center border-r border-b border-gray-800 px-4 text-sm text-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-300 radix-state-active:border-b-transparent radix-state-active:bg-slate-800 dark:border-gray-900 dark:text-slate-400 dark:focus:ring-sky-900 dark:radix-state-active:bg-slate-800"
									>
										{filename}
									</Tabs.Trigger>
								)
				...
				<div>
					{snippets.map(({ filename, content, language }, index) => (
						...
						>
							<div className="w-8 shrink-0 grow-0 bg-slate-800 pt-[19px] pb-3 text-right font-mono text-sm leading-none text-slate-300 dark:bg-[#0d1116] dark:text-slate-700">
							...
							<ScrollArea.Root className="w-full overflow-auto bg-slate-800 dark:bg-[#0D1116]">
								<ScrollArea.Viewport>
