...
export function ThemeSelector(props) {
	...
	useEffect(() => {
		if (selectedTheme) {
			...
			document
				...
				?.setAttribute('content', selectedTheme.value === 'dark' ? 'rgb(11, 15, 26)' : '#ffffff');
	...
	return (
		<Listbox as="div" value={selectedTheme || { value: 'dark' }} onChange={setSelectedTheme} {...props}>
			...
			<Listbox.Options className="absolute top-full right-1/2 mt-3 w-36 -translate-x-1/2 space-y-1 rounded-xl bg-white p-3 text-sm font-medium shadow-md shadow-black/5 ring-1 ring-black/5 dark:bg-gray-800 dark:ring-white/5">
				{themes.map((theme) => (
					<Listbox.Option
						key={theme.value}
						value={theme}
						...
						{({ selected }) => (
							<>
								<div className="rounded-md bg-white p-1 shadow ring-1 ring-gray-900/5 dark:bg-gray-700 dark:ring-inset dark:ring-white/5">
									<theme.icon
										className={clsx('h-4 w-4', selected ? 'fill-sky-400 dark:fill-sky-400' : 'fill-gray-400')}
									/>
								</div>
