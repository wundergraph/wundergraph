...
>(
	...
	resolveConfig(config)
		.then(async (resolved) => {
			// done #1 (mandatory)
			writeWunderGraphFileSync(generated, 'build_info', buildInfoJSON);
			...
			// done #2 (mandatory)
			writeWunderGraphFileSync(generated, 'schema', schemaContent, 'graphql');
			...
			if (fs.existsSync(configJsonPath)) {
				...
				if (configJSON !== existing) {
					writeWunderGraphFileSync(configJsonPath, 'config', configJSON);
				}
			} else {
				writeWunderGraphFileSync(configJsonPath, 'config', configJSON);
			}
			...
			// done #5 (mandatory)
			writeWunderGraphFileSync(generated, 'postman', postman.toJSON());
			...
			// done #6 (mandatory)
			writeWunderGraphFileSync(generated, 'openapi', openApiSpec);
		})
		.catch((e: any) => {
			//throw e;
...
const writeWunderGraphFileSync = (pathToJson: string, fileName: string, json: object | string, extension = 'json') => {
	...
	fs.writeFileSync(path.join(pathToJson, `wundergraph.${fileName}.${extension}`), json, { encoding: utf8 });
