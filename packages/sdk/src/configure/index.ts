...
const writeWunderGraphFileSync = (fileName: string, json: object | string, extension = jsonExtension) => {
	if (typeof json !== 'string') {
		json = JSON.stringify(json, null, 2);
	}

	fs.writeFileSync(path.join(generated, `wundergraph.${fileName}.${extension}`), json, { encoding: utf8 });
