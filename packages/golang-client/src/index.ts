...
import { hasInput, Logger, visitJSONSchema } from '@wundergraph/sdk/internal';
...
const gofmt = (code: string) => {
	try {
		...
		if (e instanceof Error && e.message.indexOf('ENOENT') >= 0) {
			Logger.child({ plugin: 'golang-client' }).error(
				'gofmt is not installed. If you want to prettify the generated code, please install gofmt'
			);
		} else {
