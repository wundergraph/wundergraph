...
const shopify = introspect.graphql({
	...
	url: 'https://appulse-dev.myshopify.com/api/2021-07/graphql.json',
	headers: (builder) => {
		builder.addStaticHeader('X-Shopify-Storefront-Access-Token', process.env.SHOPIFY_STOREFRONT_TOKEN || '');
		return builder;
	},
});

const postmark = introspect.openApiV2({
	id: 'postmark',
	apiNamespace: 'postmark',
	source: {
		kind: 'file',
		filePath: './postmark.yml',
	},
});

const typesense = introspect.openApiV2({
	id: 'typesense',
	apiNamespace: 'typesense',
	source: {
		kind: 'file',
		filePath: './test.yml',
	},
	baseURL: 'https://eo6j7vlrvrflemb.m.pipedream.net',
	headers: (builder) =>
		builder
			// this header has no effect on introspection
			.addClientRequestHeader('x-typesense-api-key', 'eeeeeee')
			// this header is shared between introspection and actual requests
			.addStaticHeader('x-typesense-api-key', 'eeeeeee'),
});

const petstore = introspect.openApiV2({
	id: 'petstore',
	source: {
		kind: 'file',
		filePath: './petstore.json',
	},
	apiNamespace: 'petstore',
	headers: (builder) => builder.addStaticHeader('content-type', 'application/json'),
});

// const slack = introspect.openApiV2({
// 	id: 'slack',
// 	apiNamespace: 'slack',
// 	source: {
// 		kind: 'file',
...
// const spotify = introspect.openApiV2({
// 	id: 'spotify',
// 	apiNamespace: 'spotify',
// 	source: {
// 		kind: 'file',
// 		filePath: 'spotify.yml',
// 	},
// });
...
configureWunderGraphApplication({
	...
	authentication: {
		...
		tokenBased: {
			providers: [
				{
					userInfoEndpoint: 'http://localhost:9991/operations/userInfo',
				},
			],
		},
	},
