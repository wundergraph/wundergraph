...
var upCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		configBundler := bundler.NewBundler(bundler.Config{
			Name:          "config-bundler",
			EntryPoints:   []string{configEntry},
			AbsWorkingDir: wunderGraphDir,
...
var (
	wunderGraphFactoryTemplate = template.Must(template.New(wunderGraphApplicationTs).Parse(`
...
func EnsureWunderGraphApplicationTS(wunderGraphDir string) error {
	...
	wunderGraphFactoryTsPath := filepath.Join(generated, wunderGraphApplicationTs)
	f, err := os.Create(wunderGraphFactoryTsPath)
	if err != nil {
		return fmt.Errorf("error creating %s: %s", wunderGraphFactoryTsPath, err)
	}
	defer f.Close()
	if err := wunderGraphFactoryTemplate.Execute(f, wunderGraphApplicationTemplateData{
